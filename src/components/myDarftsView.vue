<template>
  <div class="myDarftsView" :class='$store.state.isPc?"pcAuto":"mdAuto"'>
    <div class="left">
      <div class="box">
        <el-carousel 
        ...
        >
          <el-carousel-item v-for="(item,i) in review_image_ids" :key="i">
            <img :src="hasImgPrefix(item.photopath)">
          </el-carousel-item>
        </el-carousel>
        <div class="index" v-if='review_image_ids.length'>{{imgIndex}}/{{review_image_ids.length}}</div>
        <div class="content">
        <div class="head">
      ...
      <div class="btnBox">
        <button @click='toPubish()'>Pubish</button>
...
export default {
  ...
  methods:{
    ...
    toEdit(){
      this.$router.push({path:'/postComments',query:{cid:this.ceping_review_id}})
    },
    getReview(){
      ...
      return httpNetwork(url,data).then(res=>{
        this.review_image_ids = res.data.review_images
        this.title = res.data.review_title
        this.content = res.data.review_content
        this.time = res.data.date_added
